...
public class ProcessAllOrdersInvocable : IInvocable
{
    ...
    {
        var @lock = new SqlDistributedLock(nameof(ProcessAllOrdersInvocable), this.connectionString);
        ...
        if (handle != null)
        {
            ...
            watch.Stop();
            Console.WriteLine($"### {nameof(ProcessAllOrdersInvocable)} took {watch.ElapsedMilliseconds} ms. Finished at {DateTime.UtcNow.ToUniversalTime()}");
        }
    }
