...
public class ProcessAllStockItemTransactionsInvocable : IInvocable
{
    ...
    {
        var @lock = new SqlDistributedLock(nameof(ProcessAllStockItemTransactionsInvocable), this.connectionString);
        ...
        if (handle != null)
        {
            ...
            watch.Stop();
            Console.WriteLine(
                $"### {nameof(ProcessAllStockItemTransactionsInvocable)} took {watch.ElapsedMilliseconds} ms. Finished at {DateTime.UtcNow.ToUniversalTime()}");
        }
    }
